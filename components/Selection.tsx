...
function Card(props: SelectionProps) {
    return (
        <>
            <motion.li
                className={`hover:bg-blue-300 hover:ring-blue-400 hover:shadow-xl shadow-lg ring-1 ring-blue-200 bg-gradient-to-r from-blue-300 rounded-lg cursor-pointer`}
                whileHover={{ scale: 1.02 }}
                transition={{ type: 'tween' }}
            >
                <Link href={props.href}>
                    <div
                        className={`h-48 flex justify-center p-4 flex-col align-middle items-center ${Style.card_wave}`}
                    >
                        <div>
                            <Image
                                src={props.imgUrl}
