...
const CommentEdit = (
    { comment, closeHandler }: 
    { comment: CommentType, closeHandler: () => void }
)=> {
                ...
                <Flex ml={'auto'} gap={'0.5rem'}>
                   ...
                   <Button size={'md'} variant={'outline'} colorScheme="green"
                      onClick={()=> {
                         setLoading(true);
                         handleSubmit((data) => {
                            clientCommentsHandler.Update(comment, comment.id, data.content, (err)=> {
                               setLoading(false);
                               toast({ description: err, status: 'error', position: 'top' });
