...
class DiscussionHandler extends Singleton<DiscussionHandler> {
   ...
   * @param discussion
   */
  public deleteDiscussion(discussion: DiscussionDocType) {
    return deleteDoc(doc(discussionsColRef, discussion.id as string)).then(() => {
      // delete discussion comments
      getDocs(query(discussionsCommentsColRef, where('dis_id', '==', discussion.id))).then(
        (snapShot) => {
  ...
  /*
    ...
    */
  public incrementViewCount(discussion_id: string) {
    ...
    let timeDiffInMin: number = Math.abs(
      Math.round(
        (new Date().getTime() -
          new Date(
            localStorage.getItem(DISCUSSION_DATE_STORAGE_KEY) ||
              new Date().setMinutes(new Date().getMinutes() + COOL_DOWN_TIME)
          ).getTime()) /
          60000
      )
    );
