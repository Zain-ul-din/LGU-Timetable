import { DocumentReference, collection, doc, serverTimestamp, updateDoc, setDoc, arrayUnion, arrayRemove  } from "firebase/firestore";
...
export function postVote (docId: string, user_id: string, voteType: "up" | "down")
{
    ...
    updateDoc(doc(sub_collectionRef, user_id), {
        ...
        createParticipant(user_id, doc(sub_collectionRef, user_id), ()=> postVote(docId, user_id, voteType));
    });
} 
...
export function addReaction (docId: string, user_id: string, reaction: string)
{
    ...
    updateDoc(doc(sub_collectionRef, user_id), {
        ...
        createParticipant(user_id, doc(sub_collectionRef, user_id), ()=> addReaction(docId, user_id, reaction));
    });
} 
...
export function removeReaction (docId: string, user_id: string, reaction: string)
{
    ...
    updateDoc(doc(sub_collectionRef, user_id), {
        ...
        createParticipant(user_id, doc(sub_collectionRef, user_id), ()=> removeReaction(docId, user_id, reaction));
    });
}
