          ...
          <>
            <Flex flexDir={'column'} gap={5}>
              {Object.entries(subject.conflicts).map(([subjName, lectures], i) => {
                return (
                  <Flex key={i} bg={'red.400'} p={1} rounded={'md'} flexDir={'column'}>
                    CONFLICT WITH:
                    <Box fontWeight={'bold'}>{subjName}</Box> At:
                    {lectures.map((l, i) => {
                      return (
                        <Flex key={i} bg={'red.700'} p={0.5} px={2} fontWeight={'extrabold'}>
                          - {l.day}
                          <Text display={'inline-block'} mx={2}>
                            <TimeRenderer time={l.time} />
                          </Text>
                        </Flex>
                      );
                    })}
...
const TimeRenderer = ({ time }: { time: LectureTimeObjectType }) => {
  return (
    <>
      {time.startTime.hours.toString().padStart(2, '0')}:
      {time.startTime.minutes.toString().padStart(2, '0')}
      {' - '}
      {time.endTime.hours.toString().padStart(2, '0')}:
      {time.endTime.minutes.toString().padStart(2, '0')}
    </>
  );
};

const CourseCardMemo = memo(CourseCard, (acc, curr) => {
  if (JSON.stringify(acc.subject) == JSON.stringify(curr.subject)) return true;
  return false;
});
