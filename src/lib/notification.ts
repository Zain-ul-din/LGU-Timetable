...
export class DiscussionNotification {
  ...
  private static collection: CollectionReference = collection(
   ...
   */
  constructor(subscriber_id: string) {
   ...
   */
  subscribe(observer: (snapShort: QuerySnapshot) => void): Unsubscribe {
    const q = query(
      DiscussionNotification.collection,
   ...
   * @param {*} observer_id
   */
  unSubscribe(discussion_id: string) {
   ...
   */
  static notify(sender: string, discussion_id: string, subscribers: Array<string>) {
    const _doc = doc(DiscussionNotification.collection, discussion_id);
...
export const useDiscussionNotifications = (user: UserDocType | null) => {
