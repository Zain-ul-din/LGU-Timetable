...
async function triggerWorkFlow(uid: string, session_id: string) {
  ...
  const GITHUB_URL =
    'https://api.github.com/repos/Zain-ul-din/lgu-crawler/actions/workflows/89946576/dispatches';
...
async function canTriggerWorkFlow(quota: WorkFlowQuotaType, user: UserDocType) {
  ...
  const timeSpanSinceLastUpdate =
    new Date().getTime() - fromFirebaseTimeStamp(last_updated).getTime();
  ...
  if (timeSpanSinceLastUpdate < coolDownTime) {
    const remainingCoolDownTime =
      (((coolDownTime - timeSpanSinceLastUpdate) / ONE_HOUR_IN_MS) * 60) | 0;
    return {
